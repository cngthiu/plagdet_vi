...
# (BỔ SUNG RERANKER + LỌC/ƯU TIÊN THEO CE)
# ============================================
...
from ..scoring.aggregate import MatchSpan, aggregate
from ..report.html import save_report

def _read_any(path: str, use_ocr: bool) -> str:
    p = path.lower()
    if p.endswith(".pdf"):
        return read_pdf_text(path, ocr=use_ocr)
    if p.endswith(".docx"):
...
def _prepare_windows(text: str, cfg: Settings) -> tuple[list[str], list[str]]:
    ...
    wins = window_paragraphs(blocks["paras"], cfg.win_min_tokens, cfg.win_max_tokens)
...
def compare_paths(path_a: str, path_b: str, out_dir: str, cfg: Settings) -> dict:
    ...
    if not paras_a or not paras_b:
        raise RuntimeError("Empty content after preprocessing.")
    ...
    if rr_model is not None and pairs:
        texts = [(paras_a[i], paras_b[j]) for (i, j, _) in pairs]
        rr_scores = rerank_pairs(rr_model, texts)
        # Ghép CE score vào pairs → lọc theo rerank_min → sắp xếp ưu tiên CE
        pairs = [
            (i, j, cos, rr)
            for (rr, (i, j, cos)) in zip(rr_scores, pairs)
            if rr >= cfg.rerank_min
        ]
        pairs.sort(key=lambda x: (x[3], x[2]), reverse=True)  # ưu tiên CE rồi cosine
    else:
        # không có CE: chỉ sắp xếp theo cosine
    ...
    for (i, j, cos, rr) in pairs:
        jac5 = jaccard(j5_a[i], j5_b[j])
        jac7 = jaccard(j7_a[i], j7_b[j])
        ...
        a_toks = paras_a[i].split()
        b_toks = paras_b[j].split()
        ...
        spans.append(MatchSpan(
            a_idx=i, b_idx=j,
            a_span=(ai0, ai1), b_span=(bj0, bj1),
            cos=cos, jac5=jac5, jac7=jac7,
            simhash_hamm=hamm, sw_score=sw, rerank=rr
        ))

    # Aggregate + report
    rep = aggregate(paras_a, spans)
    path = save_report(out_dir, rep, paras_a, paras_b)
    return {
        "verdict": rep.verdict,
        "score": rep.score_0_100,
        "coverage": rep.coverage_ratio,
